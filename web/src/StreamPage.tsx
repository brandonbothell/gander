...
export default function StreamPage({
  ...
  onSessionMonitorClosed,
  logout,
}: StreamPageProps) {
  // --- New state for dynamic streams ---
  const [streams, setStreams] = useState<Stream[]>([]);
  const [streamOrder, setStreamOrder] = useLocalStorageState<string[]>(
    'streamOrder',
    [],
  );
  ...
  const [isMotionRecordingPaused, setMotionRecordingPaused] = useState<{
    [streamId: string]: boolean;
  }>({});
  const [nicknames, setNicknames] = useState<{ [filename: string]: string }>(
    {},
  );
  const [shouldNotifyOnMotion, setShouldNotifyOnMotion] =
    useLocalStorageState<boolean>('motionNotify', false);
  ...
  const [isLoadingMotionNotifications, setIsLoadingMotionNotifications] =
    useState(true);
  ...
  const [signedThumbUrls, setSignedThumbUrls] = useLocalStorageState<{
    [id: string]: { url: string; expires: number };
  }>('signedLiveThumbUrls', {});
  const [isVideoPaused, setIsVideoPaused] = useState(false);
  const [deletedRecordings, setDeletedRecordings] = useState<{
  ...
  const [totalRecordings, setTotalRecordings] = useState<{
  ...
  const [isRecordingsListScrolling, setIsRecordingsListScrolling] =
    useState(false);
  const [recordingsListBottomVisible, setRecordingsListBottomVisible] =
    useState(false);
  ...
  const debugLongPressTimeout = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const [lowDiskModalStreamId, setLowDiskModalStreamId] = useState<
    string | null
  >(null);
  const [lowDiskLastDismissed, setLowDiskLastDismissed] = useLocalStorageState<
    Record<string, number>
  >('lowDiskLastDismissed', {});
  ...
  function handleLowDiskModalClose() {
    if (lowDiskModalStreamId) {
      setLowDiskLastDismissed((prev) => ({
        ...(prev || {}),
        [lowDiskModalStreamId]: Date.now(),
      }));
    }
  ...
  const filteredRecordings = useMemo(() => {
    ...
    if (
      ...
      const sorted = notDeleted.sort((a, b) =>
    ...
    return immediateResults.sort((a, b) =>
      b.filename.localeCompare(a.filename),
    );
  }, [
    cachedRecordings,
    ...
    userTyping,
    frozenFilteredRecordings,
    filteredRecordingsCache,
    isSearching,
  ]);
        ...
        if (Hls.isSupported()) {
          ...
          hls.on(Hls.Events.ERROR, async (_event, data) => {
            ...
            if (
              ...
              console.warn(
                'HLS 403 Forbidden: refreshing signed stream URL...',
              );
              setIsLoadingStream(true);
              hls.destroy();
              delete videoRef.current?._hls;
            ...
            if (data.fatal) {
              ...
              switch (data.details) {
                case Hls.ErrorDetails.BUFFER_APPEND_ERROR:
                  console.log(
                    'Buffer append error, clearing buffers and restarting...',
                  );
                  try {
                    ...
                    console.error(
                      'Error recovering from buffer append error:',
                      error,
                    );
          ...
          const handleInitialPlayback = () => {
            ...
            if (
              ...
              console.log(
                'HLS initial playback started for stream:',
                activeStream.id,
              );
              setIsLoadingStream(false);
          ...
          setTimeout(() => {
            ...
            if (!hasStartedPlaying) {
              console.log(
                'Fallback: setting loading to false after timeout for stream:',
                activeStream.id,
              );
              setIsLoadingStream(false);
            }
          ...
          hls.on(Hls.Events.FRAG_LOADED, () => {
            ...
            if (video && hls.liveSyncPosition !== undefined) {
              ...
              if (latency > 15) {
                console.log(
                  `Latency too high (${latency.toFixed(1)}s), jumping to live`,
                );
                seekToLiveEdge(videoRef, hls);
              }
            }
  ...
  }, [activeStream, setStreamOrder, streams.length]);
  ...
  const loadPage = useCallback(
    async function loadPage(stream: Stream, page: number, force = false) {
      ...
      const res = await authFetch(
        `${API_BASE}/api/recordings/${stream.id}/${page}`,
      );
      if (!res.ok) return;
      ...
      if (data.deletedRecordings && Array.isArray(data.deletedRecordings)) {
        setDeletedRecordings((prev) => ({
          ...prev,
          [stream.id]: [
            ...new Set([...(prev[stream.id] || []), ...data.deletedRecordings]),
          ],
        }));
      }
      ...
      if (lastSeenRecording[stream.id]) {
        const idx = newRecs.findIndex(
          (r) => r.filename === lastSeenRecording[stream.id]!,
        );
        if (idx !== -1) {
          foundLastSeen = true;
        } else if (
          newRecs.length > 0 &&
          newRecs[newRecs.length - 1].filename.localeCompare(
            lastSeenRecording[stream.id]!,
          ) < 0
        ) {
          foundLastSeen = true;
        }
    ...
    [reachedLastSeen, activeStream, lastSeenRecording, setCachedRecordings],
  );
  ...
  useEffect(() => {
    ...
    function onScroll() {
      ...
      const totalRemaining =
        (totalRecordings[recordingsStream.id] || 0) -
        (cachedRecordings[recordingsStream.id]?.length || 0);
      const isNearBottom =
        list!.scrollTop + list!.clientHeight >= list!.scrollHeight - 300;
      ...
      if (
        ...
        alert(
          "Reached the end of the recordings list, and you're missing clips! Please forgive us while we reload the recordings list.",
        );
      }
    }
  ...
  useEffect(() => {
    const list = recordingsListRef.current;
    if (!list || isLoadingMore || !recordingsListOpen) return;
  ...
  useEffect(() => {
    ...
    window.addEventListener('touchmove', checkAndScrollToRecordingsList, {
      passive: true,
    });
    ...
    function handleScroll() {
      ...
      if (
        ...
        window.innerHeight + currentScrollY <
          document.body.offsetHeight - window.innerHeight * 0.1 &&
        recordingsListOpen
      ) {
  ...
  useEffect(() => {
    ...
    if (sentinel && !isMobileWidth) {
      // Add !isMobile check
      observer = new window.IntersectionObserver(updateSticky, {
        threshold: 0,
      });
  ...
  useEffect(() => {
    ...
    if (sentinel && isMobileWidth) {
      // Add isMobile check
      observer = new window.IntersectionObserver(updateMobileSticky, {
        threshold: 0,
      });
      observer.observe(sentinel);
    }
  ...
  useEffect(() => {
    ...
    const recordingsStream = viewingRecordingsFrom ?? activeStream;
    const cachedLen = recordingsStream
      ? cachedRecordings[recordingsStream.id]?.length || 0
      : 0;
      ...
      const pollLatest = async () => {
        ...
        preCacheTimeout = setTimeout(async () => {
          ...
          if (cachedLen < total) {
            ...
            console.log(
              `Pre-caching page ${nextPage} for stream ${recordingsStream.id}`,
            );
            setIsLoadingMore(true);
            setCurrentPage(nextPage);
  ...
  async function pollLatestRecordings(stream: Stream, cancelled?: boolean) {
    ...
    try {
      const res = await authFetch(
        `${API_BASE}/api/latest-recordings/${stream.id}`,
      );
      if (!res.ok) return;
      const data = await res.json();
      ...
      if (
        data.deletedRecordings &&
        Array.isArray(data.deletedRecordings) &&
        data.deletedRecordings.length > 0
      ) {
        setDeletedRecordings((prev) => ({
          ...prev,
          [stream.id]: [
        ...
        .then(
          (status: {
            ...
            for (const [streamId, s] of Object.entries(status)) {
              const recordingJustStarted =
                s.recording && Date.now() - s.startedRecordingAt < 1000; // 1 second threshold

              // Play sound when ANY stream starts recording
              if (recordingJustStarted) {
                console.log(
                  `Motion detected on stream ${streamId}, playing sound`,
                );
                playMotionSound();
  ...
  useEffect(() => {
    ...
    if (streamStatus?.recording && streamStatus.startedRecordingAt > 0) {
      ...
      if (timeSinceStart <= 1000) {
        // 1 second
        console.log(
          `Switched to stream ${activeStream.id} that recently started recording (${Math.floor(timeSinceStart / 1000)}s ago)`,
        );
        playMotionSound();
      } else {
        console.log(
          `Switched to stream ${activeStream.id} that started recording ${Math.floor(timeSinceStart / 1000)}s ago (too old)`,
        );
      }
    } else {
      console.log(
  ...
  useEffect(() => {
    if (location.state) {
      ...
      if (state.scrollToRecordings && recordingsListRef.current) {
        setTimeout(() => {
          recordingsListRef.current?.scrollIntoView({
            behavior: 'smooth',
            block: 'center',
          });
      ...
      if (state.streamId) {
        ...
        window.history.replaceState({ ...state, streamId: undefined }, '');
      ...
      if (state.recordingBeingViewed) {
        ...
        if (
          !viewed.find(
            (viewed) =>
              viewed.filename === filename && viewed.streamId === streamId,
          )
        ) {
  ...
  useEffect(() => {
    if (
      (isRecordingsListScrolling || pullDistance) &&
      ...
      if (shouldNotifyOnMotion) {
        ...
        if (Capacitor.getPlatform() !== 'web') {
          ...
          try {
            ...
            setErrorModalMsg(
              'Failed to enable motion notifications. Please check your device settings.',
            );
          ...
          if (!navigator.serviceWorker) {
            ...
            setShouldNotifyOnMotion(false);
            setErrorModalMsg(
              'Service Worker not supported. Please use a modern browser to enable notifications.',
            );
            setErrorModalOpen(true);
  ...
  function fetchSignedThumbUrls() {
    Object.values(streams).forEach((stream) => {
      ...
      if (!entry || entry.expires < now + 10) {
        fetchWithRetry(() =>
          authFetch(`${API_BASE}/api/signed-latest-thumb-url/${stream.id}`),
        )
          .then(async (response) => {
          ...
          .catch((err) => {
            console.error('Failed to fetch signed thumb URL:', err);
          });
      }
    });
  }
  ...
  useEffect(() => {
    ...
    authFetch(`${API_BASE}/api/recordings-nicknames/${recordingsStream.id}`)
      .then((res) => res.json())
      .then(
        (nicknamedRecordings: Array<{ filename: string; nickname: string }>) =>
          setNicknames(
            nicknamedRecordings.reduce((acc: Record<string, string>, rec) => {
              acc[rec.filename] = rec.nickname;
              return acc;
            }, {}),
          ),
      )
  ...
  useEffect(() => {
    ...
    async function fetchDeleted() {
      ...
      const recordingsStream = viewingRecordingsFrom ?? activeStream;
      const res = await authFetch(
        `${API_BASE}/api/deleted-recordings/${recordingsStream.id}`,
      ).catch(() => ({ ok: false }) as { ok: false });
      if (!res.ok) return;
      const list: string[] = await res.json();
      if (
        !cancelled &&
        (!deletedRecordings[recordingsStream.id] ||
          !list.every((r) =>
            deletedRecordings[recordingsStream.id].includes(r),
          ))
      ) {
        setDeletedRecordings((prev) => ({
          ...prev,
          [recordingsStream.id]: list,
        }));
      }
    }
    fetchDeleted();
  ...
  function handleImmediateDeleteUpdate(deletedFilenames: string[]) {
    ...
    setCachedRecordings((prev) => {
      const updated = (prev[viewingStream.id] || []).filter(
        (r) => !deletedFilenames.includes(r.filename),
      );
      return {
        ...prev,
        [viewingStream.id]: updated.sort((a, b) =>
          b.filename.localeCompare(a.filename),
        ),
      };
    });
      ...
      if (currentLastSeen && deletedFilenames.includes(currentLastSeen)) {
        ...
        const newLastSeen =
          remainingRecordings.length > 0
            ? remainingRecordings[0].filename
            : null;
  ...
  useEffect(() => {
    ...
    setCachedRecordings((prev) => {
      ...
      Object.entries(deletedRecordings).forEach(([streamId, deletedList]) => {
        ...
        updatedPrev[streamId] = updated.sort((a, b) =>
          b.filename.localeCompare(a.filename),
        );
      });
      return updatedPrev;
  ...
  async function fetchMasks() {
    ...
    const res = await authFetch(
      `${API_BASE}/api/masks/${activeStream.id}`,
    ).catch(() => ({ ok: false }) as { ok: false });
    if (!res.ok) return;
    const data = await res.json();
  ...
  useEffect(() => {
    ...
    function updateVideoSize(event: Event | null) {
      ...
      if (
        ['canplay', undefined].includes(event?.type) &&
        video.clientWidth > 0 &&
  ...
  function openAndScrollToRecordingsList() {
    ...
    setTimeout(
      ...
      recordingsListOpen ? 0 : 400,
    ); // Wait for open animation if needed
  }
  ...
  function getThumbUrl(stream: Stream) {
    ...
    if (signedThumbUrls[stream.id]?.url) {
      ...
      return (
        signedThumbUrls[stream.id].url +
        (signedThumbUrls[stream.id].url.includes('?') ? '&' : '?') +
        't=' +
        throttledTimestamp
      );
    }
  ...
  const handleSaveSettings = async (
    ...
    if (
      typeof newFfmpegInput === 'string' &&
      newFfmpegInput.trim() &&
      newFfmpegInput !== stream.ffmpegInput
    ) {
      patch.ffmpegInput = newFfmpegInput.trim();
    }
    if (
      typeof newRtspUser === 'string' &&
      newRtspUser.trim() !== (stream.rtspUser || '')
    ) {
      patch.rtspUser = newRtspUser.trim() || undefined;
    }
    if (
      typeof newRtspPass === 'string' &&
      newRtspPass.trim() !== (stream.rtspPass || '')
    ) {
    ...
    <div className="App with-side-padding">
      ...
      <div style={{ userSelect: 'none' }}>
          ...
          <RecordingBar
            ...
            onNavigate={(filename) => {
              ...
              setRecordingBeingViewed(
                recordingBeingViewed
                  ? { streamId: recordingBeingViewed.streamId, filename }
                  : null,
              );
              setOpeningRecording(false);
              const recordingsStream = viewingRecordingsFrom ?? activeStream;
            ...
            setAutoScrollUntilRef={(until) => {
              autoScrollUntilRef.current = until;
            }}
            ...
            {showMaskEditor && (
              <MaskEditorOverlay
                ...
                saveMaskPosition={async (maskId, newPos) => {
                  ...
                  await authFetch(
                    `${API_BASE}/api/masks/${activeStream.id}/${maskId}`,
                    {
                      method: 'PATCH',
                      headers: { 'Content-Type': 'application/json' },
        ...
        <StreamTilesGrid
          ...
          onDeleteStream={async (stream) => {
            try {
              const res = await authFetch(
                `${API_BASE}/api/streams/${stream.id}`,
                {
                  method: 'DELETE',
                },
              );
              ...
              if (!res.ok) {
                ...
                alert(
                  `Failed to delete stream: ${error.message || 'Unknown error'}`,
                );
                return;
              }
          ...
          onToggleMotionPause={async (stream, motionRecordingEnabled) => {
            const response = await authFetch(
              `${API_BASE}/api/motion-pause/${stream.id}`,
              {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
          ...
          motionSaving={Object.entries(motionStatus).reduce(
            (prev, e) => ({ ...prev, [e[0]]: e[1].saving }),
            {},
          )}
        />
        ...
        <div
          style={
            isMobileWidth ? {} : { display: 'flex', justifyContent: 'flex-end' }
          }
        >
          <div
            className={
          ...
          >
            <SearchTools
              ...
              onFocusSearchInput={
                isMobileWidth ? () => setForceSticky(true) : undefined
              }
              onBlurSearchInput={
                isMobileWidth ? () => setForceSticky(false) : undefined
              }
      ...
      <div
        ...
        style={{
          ...
          overflowY: transferScrollToPage
            ? 'hidden'
            : recordingsListOpen
              ? 'auto'
              : 'hidden', // <-- updated
      ...
      >
        gander © {new Date().getFullYear()} Brandon Bothell. All rights
        reserved.
      </div>
      <div
        style={{
        ...
        >
          <FiChevronDown
            ...
            style={{
              ...
              animation:
                'bounceDown 5s infinite cubic-bezier(.4,2,.6,1), pulseDropShadow 5s infinite cubic-bezier(.4,2,.6,1)',
            }}
          />
          <div
      ...
      {activeStream && (
        <FloatingMenuPopout
          ...
          cachedRecordings={
            cachedRecordings[viewingRecordingsFrom?.id ?? activeStream.id]
          }
          setRecordings={(recs) => {
            if (!activeStream) return;
            ...
            setCachedRecordings((prev) => ({
              ...prev,
              [viewingStream.id]: recs.sort((a, b) =>
                b.filename.localeCompare(a.filename),
              ),
            }));
            setTotalRecordings((prev) => ({
              ...prev,
              [viewingStream.id]: Math.max(
                0,
                (prev[viewingStream.id] || 0) - selected.length,
              ),
            }));
      ...
      <AddStreamModal
        ...
        onStreamCreated={(newStreamData) => {
          setStreams((streams) =>
            streams
              ? [...streams, newStreamData as Stream]
              : [newStreamData as Stream],
          );
        }}
        authFetch={authFetch}
        API_BASE={API_BASE}
      ...
      <StreamSettingsModal
        ...
        onReconnect={async (stream) => {
          ...
          setIsLoadingStream(true);
          try {
            await authFetch(`${API_BASE}/api/streams/${stream.id}/reconnect`, {
              method: 'POST',
            });
...
function seekToLiveEdgeGentle(
  ...
  if (hls && hls.liveSyncPosition !== undefined && hls.liveSyncPosition !== null) {
    ...
    console.log(
      `Initial gentle seek to HLS live edge: ${targetTime.toFixed(2)}s`,
    );
    ...
    console.log(
      `Initial gentle seek to duration-based live edge: ${targetTime.toFixed(2)}s`,
    );
    video.currentTime = Math.max(0, targetTime);
  } else if (video.seekable && video.seekable.length > 0) {
    const targetTime = video.seekable.end(video.seekable.length - 1) - 2;
    console.log(
      `Initial gentle seek to seekable live edge: ${targetTime.toFixed(2)}s`,
    );
    video.currentTime = Math.max(0, targetTime);
  }
}
...
function seekToLiveEdge(
  ...
  if (hls && hls.liveSyncPosition !== undefined && hls.liveSyncPosition !== null) {
    ...
    console.log(
      `Seeking to duration-based live edge: ${targetTime.toFixed(2)}s`,
    );
    video.currentTime = Math.max(0, targetTime);
  } else if (video.seekable && video.seekable.length > 0) {
...
function mergeMasks(
  serverMasks: StreamMask[],
  clientMasks: ClientMask[],
): ClientMask[] {
