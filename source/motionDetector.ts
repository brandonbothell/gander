...
const streamMotionThresholds = {
  min: 0.002,
  max: 0.4, // Slightly higher due to lower resolution
}
...
async function getMasksForStream(
  streamId: string,
  ...
  return runFfmpegTask(
    () =>
      new Promise((resolve, reject) => {
        debugLog(
          `[Motion] Extracting frame from ${segmentPath} to ${outputPath}`,
        )
        ...
        fs.promises
          ...
          .then((stats) => {
            if (stats.size < 1000) {
              logMotion(
                `[Motion] Segment file too small: ${segmentPath}`,
                'warn',
              )
            ...
            const ffmpeg = spawn(
              ...
              [
                ...
                segmentPath,
                '-vf',
                `scale=${STANDARD_WIDTH}:${STANDARD_HEIGHT}`,
                '-vframes',
                '1',
                '-update',
                '1',
              ...
              setTimeout(async () => {
                if (code === 0) {
                  ...
                  try {
                    ...
                    if (stats.size > 100) {
                      debugLog(
                        `[Motion] Successfully extracted frame (${stats.size} bytes)`,
                      )
                      resolve()
                    } else {
                      logMotion(
                        `[Motion] Frame file too small (${stats.size} bytes), retrying with PNG`,
                        'warn',
                      )
                      ...
                      pngFfmpeg.on('close', async (pngCode) => {
                        ...
                        try {
                          if (
                            pngCode === 0 &&
                            (await fs.promises.stat(pngPath)).size > 100
                          ) {
                            try {
                      ...
                      pngFfmpeg.on('error', (err) => {
                        clearTimeout(pngTimeout)
                        logMotion(
                          `[Motion] PNG FFmpeg spawn error: ${err}`,
                          'error',
                        )
                        reject(err)
                      })
                    ...
                    logMotion(
                      `[Motion] No output file found at ${outputPath} after FFmpeg success`,
                      'error',
                    )
                    ...
                    if (DEBUG_MOTION) {
                      try {
                        ...
                        debugLog(
                          `[Motion] Directory contents: ${files.slice(0, 10).join(', ')}${files.length > 10 ? '...' : ''}`,
                        )
                      } catch (e) {
                  ...
                  logMotion(
                    `[Motion] FFmpeg failed with code ${code}: ${errorOutput}`,
                    'error',
                  )
...
export async function detectMotion(
  ...
  try {
    ...
    try {
      ...
      if (stat.size < 1000) {
        logMotion(
          `[${streamId}] [Motion] Segment file too small: ${segmentPath}`,
          'warn',
        )
        return { motion: false, aboveCameraMovementThreshold: false }
      }
      ...
      logMotion(
        `[${streamId}] [Motion] Segment file missing: ${segmentPath}`,
        'error',
      )
      return { motion: false, aboveCameraMovementThreshold: false }
    }
    ...
    try {
      ...
      logMotion(
        `[${streamId}] Failed to extract frame from ${segmentPath}: ${error}`,
        'error',
      )
      return { motion: false, aboveCameraMovementThreshold: false }
    }
    ...
    if (!frameExists) {
      logMotion(
        `[${streamId}] [Motion] Frame extraction failed - no output file at ${framePath}`,
        'error',
      )
      return { motion: false, aboveCameraMovementThreshold: false }
    }
    ...
    try {
      ...
      debugLog(
        `[${streamId}] [Motion] Successfully loaded frame ${currentFrame.bitmap.width}x${currentFrame.bitmap.height}`,
      )

      // Resize if needed (sometimes FFmpeg doesn't scale properly)
      if (
        currentFrame.bitmap.width !== STANDARD_WIDTH ||
        currentFrame.bitmap.height !== STANDARD_HEIGHT
      ) {
      ...
      for (const region of masks) {
        ...
        for (
          let y = region.y;
          y < region.y + region.h && y < STANDARD_HEIGHT;
          y++
        ) {
          for (
            let x = region.x;
            x < region.x + region.w && x < STANDARD_WIDTH;
            x++
          ) {
      ...
      if (lastFrame[streamId]) {
        ...
        if (
          ...
          CAMERA_MOVEMENT_HISTORY_LENGTH
        ) {
        ...
        // Detection logic
        const movementCount =
          streamMovementHistory[streamId].filter(Boolean).length
        ...
        motionDetected =
          (streamMovementHistory[streamId].length ===
            CAMERA_MOVEMENT_HISTORY_LENGTH &&
            movementCount >= 4) ||
          (streamMotionHistory[streamId].length === GLOBAL_HISTORY_LENGTH &&
            motionCount >= 2)

        // Log motion detection results - always show actual motion, debug for all attempts
        if (motionDetected || diffPercent > 0.02) {
          logMotion(
            `[${streamId}] diff=${(diffPercent * 100).toFixed(2)}% motion=${motionDetected} threshold=${isAboveDiffThreshold} camMove=${aboveCameraMovementThreshold} (mov:${movementCount}/${streamMovementHistory[streamId].length}, mot:${motionCount}/${streamMotionHistory[streamId].length})`,
          )
        } else {
          debugLog(
            `[${streamId}] [Motion] diff=${(diffPercent * 100).toFixed(2)}% motion=${motionDetected} threshold=${isAboveDiffThreshold} camMove=${aboveCameraMovementThreshold} (mov:${movementCount}/${streamMovementHistory[streamId].length}, mot:${motionCount}/${streamMotionHistory[streamId].length})`,
          )
        }
      } else {
        debugLog(
          `[${streamId}] [Motion] No previous frame - storing first frame`,
        )
      }
...
export function cleanFrameCache(
  ...
  for (const streamId in dynamicStreams) {
    ...
    fs.readdir(stream.config.hlsDir, async (err, files) => {
      if (err) {
        console.error(`[${streamId}] Error reading HLS directory: ${err}`)
        return
      }
      // Clean up frame file
      ...
      if (motionJpgs?.length > 0) {
        logMotion(
          `[${streamId}] Deleting old motion frames: ${motionJpgs.join(', ')}`,
        )
        const toDelete = motionJpgs.map((f) =>
          path.join(stream.config.hlsDir, f),
        )
        safeUnlink(...toDelete)
      }
    })
