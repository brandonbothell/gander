...
export function useLightboxZoom({
    ...
    (deltaScale: number, clientX?: number, clientY?: number) => {
      setZoomState((prev) => {
        const nextScale = Math.max(
          1,
          Math.min(maxScale, prev.scale + deltaScale),
        )
        if (nextScale === 1) {
        ...
        if (image && clientX !== undefined && clientY !== undefined) {
          ...
          const offsetX =
            clientX - (rect.left + rect.width / 2) + prev.translateX
          const offsetY =
            clientY - (rect.top + rect.height / 2) + prev.translateY
        ...
        return clampTranslate(
          { scale: nextScale, translateX, translateY, isZoomed: true },
          image,
        )
      })
    },
  ...
  const setImageRef = useCallback((node: HTMLImageElement | null) => {
    ...
    node?.addEventListener('wheel', handleNativeWheel.current, {
      passive: false,
    })
  }, [])
