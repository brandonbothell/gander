...
import { CollapsedLightboxContextProvider } from './CollapsedLightbox.context'
...
const DEFAULT_LABELS: LightboxLabels = {
  ...
  thumbnailLabel: (index: number) => `Go to slide ${index}`,
...
export const LightboxRoot = factory<LightboxRootFactory>((_props) => {
      ...
      Object.entries(emblaOptions.breakpoints).map(([query, value]) => {
        ...
        return [
          ...
          watchDrag === undefined
            ...
            : {
                ...rest,
                watchDrag: createBreakpointWatchDrag(
                  query,
                  zoomIsActive,
                  breakpointWatchDragRef,
                ),
              },
        ]
      }),
  ...
  const { toggle: toggleFullscreenFn, fullscreen: isFullscreen } =
    useFullscreenDocument()
    ...
    exitDuration:
      typeof transitionProps?.exitDuration === 'number'
        ...
        : typeof transitionProps?.duration === 'number'
          ...
          : transitionDuration,
    exitDelay:
      typeof transitionProps?.exitDelay === 'number'
        ? transitionProps.exitDelay
        : 0,
  })
  ...
  return (
    <OptionalPortal withinPortal={withinPortal}>
      <CollapsedLightboxContextProvider value={contextValue}>
        <RemoveScroll enabled={shouldLockScroll}>
          <Box {...getStyles('root')}>
            <Transition mounted={opened} {...overlayTransition}>
              {(overlayStyles) => (
                <Box {...getStyles('overlay', { style: overlayStyles })} />
              )}
            </Transition>
      ...
      </CollapsedLightboxContextProvider>
    </OptionalPortal>
  )
})
