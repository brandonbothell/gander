...
import { type LightboxCustomSlide } from '@mantine/lightbox'
import {
  type BoxProps,
  Collapse,
  ...
  (props) => {
    ...
    const thumbnails = ctx.slides.map((slide, index) => {
      ...
      const thumbSrc =
        ...
        (slide.type === 'image' || slide.type === undefined
          ? slide.src
          : undefined)
      ...
      return (
        ...
        >
          {(slide as LightboxCustomSlide).renderThumb ? (
            (slide as LightboxCustomSlide).renderThumb!()
          ) : thumbSrc ? (
            <img
    ...
    return (
      ...
      >
        <div
          {...ctx.getStyles('thumbnailsViewport', stylesApiProps)}
          ref={thumbsRef}
        >
          <div {...ctx.getStyles('thumbnailsContainer', stylesApiProps)}>
            {thumbnails}
          </div>
        </div>
      </Collapse>
